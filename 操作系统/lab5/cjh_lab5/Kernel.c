...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          -- ??? whether needing to set 0?  A:need to but not neccessary:it may be covered by next need_num.
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      var
        ...
        oldAddrSpace: ptr to AddrSpace
        initPC: int
        open_file: ptr to OpenFile
        ...
        if i < 0
          return -1
        endIf
        -- no need to create new PCB
        
        open_file = fileManager.Open(&strBuffer)
        ...
        if open_file != null
          ...
          newAddrSpace.Init()
          oldAddrSpace = &currentThread.myProcess.addrSpace
          initPC = open_file.LoadExecutable(&newAddrSpace)
          currentThread.myProcess.addrSpace = newAddrSpace
          --free frame
          frameManager.ReturnAllFrames(oldAddrSpace)
          
          fileManager.Close(open_file)
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      var
        ...
        print("filename = ")
        i = currentThread.myProcess.addrSpace.GetStringFromVirtual (&strBuffer, filename asInteger , MAX_STRING_SIZE)
        for i = 0 to (strBuffer arraySize) - 1
          printChar(strBuffer[i])
        endFor
        nl()
        return 4000
    endFunction
...
function InitFirstProcess()
  var UserProgram: ptr to Thread
  
    UserProgram = threadManager.GetANewThread()
    UserProgram.Init("UserProgram")
...
function StartUserProcess(p: int)
  var 
    ...
    open_file = fileManager.Open("TestProgram1")
    initPC = open_file.LoadExecutable(&PCB.addrSpace)
    fileManager.Close(open_file)
