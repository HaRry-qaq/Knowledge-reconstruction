...
def train(epoch, train_loader):
    ...
    for batch_idx, data in enumerate(train_loader, 0):
        ...
        outputs1 = model1(inputs, True)
        outputs2 = model2(inputs, True)
        ...
        if batch_idx % 50 == 49:
            ...
            y.append(loss1.item() + loss2.item())
            print('[epoch:%d, batch_idx: %d] loss:%f' % (epoch + 1, batch_idx + 1, (loss1.item() + loss2.item()) / 2))
            # running_loss = 0.0
...
def test(test_loader, epoch):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            inputs,target=data
            inputs,target=inputs.to(device),target.to(device)
            outputs1=model1(inputs, False)
            outputs2=model2(inputs, False)
...
if __name__ =='__main__':
    ...
    for train_index , test_index in kf.split(dataFold):  # split 
        ...
        model1.to(device)
        optimizer1 = optim.SGD(model1.parameters(), lr=0.05, momentum=0.7 ,weight_decay=1e-4)
        ...
        model2.to(device)
        optimizer2 = optim.SGD(model2.parameters(), lr=0.05, momentum=0.7 ,weight_decay=1e-4)
